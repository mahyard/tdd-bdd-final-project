...
class TestProductRoutes(TestCase):
    ...
    def test_update_product(self):
        ...
        response = self.client.put(BASE_URL, json=data, content_type="application/json")
        self.assertEqual(response.status_code,  status.HTTP_200_OK)
    ...
    def test_delete_product(self):
        ...
        response = self.client.delete(f"{BASE_URL}/{first_product_id}")
        self.assertEqual(response.status_code,  status.HTTP_204_NO_CONTENT)
        self.assertEqual(len(response.data), 0)
        # make sure they are deleted
        response = self.client.get(f"{BASE_URL}/{first_product_id}")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_query_by_name(self):
        ...
        for prod in products:
            if prod.name == test_name:
                test_count +=1
        response = self.client.get(f"{BASE_URL}/{test_name}")
        self.assertEqual(response.status_code,  status.HTTP_200_OK)
        result = response.get_json()
    ...
    def test_query_by_category(self):
        ...
        test_count = 0
        for prod in products:
            if prod.category.name == test_category:
                test_count +=1
    ...
    def test_query_by_availability(self):
        ...
        for prod in products:
            if prod.available == True:
                test_count +=1
        response = self.client.get(f"{BASE_URL}?available=yes")
        self.assertEqual(response.status_code,  status.HTTP_200_OK)
        result = response.get_json()
        self.assertEqual(len(result), test_count)
        for item in result:
